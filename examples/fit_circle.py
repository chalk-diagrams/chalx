...
def main():
    ...
    for i in range(1, STEPS + 1):
        ...
        if i == 1 or i % 20 == 0 or i == STEPS:
            print(f"step {i:3d}  loss={float(loss_jit(params, goal)):.3f}  params={onp.asarray(params)}")

    final = render(params)
    to_png(final, "/opt/cursor/artifacts/fit_circle_final.png")
    strip = jnp.concatenate([goal, start_img, final], axis=1)
    to_png(strip, "/opt/cursor/artifacts/fit_circle_compare.png")
